...
const DashboardDr: React.FC = () => {
    ...
    const fetchMyReports = useCallback(async () => {
        ...
        if (!token) return;
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/doctor/reports/me`, {
    ...
    const submitReport = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/doctor/records/${recordId}/diagnose`, { 
                method: 'PUT', 
                headers: { 
            ...
            if (res.ok) {
                ...
                // Refresh data
                const patientsRes = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/doctor/my-patients`, { headers: { 'Authorization': `Bearer ${token}` } });
                if (patientsRes.ok) { 
    ...
    const handleViewHistory = async (patientId: string, name: string) => {
        ...
        if (!token) return;
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/doctor/patients/${patientId}/history`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
                const data = await res.json();
    ...
    const openChat = async (partnerId: string) => {
        ...
        if (token) {
            ...
            await fetch(`${import.meta.env.VITE_API_URL}/api/v1/chats/read/${partnerId}`, { method: 'PUT', headers: { 'Authorization': `Bearer ${token}` }});
            fetchChatData(token);
        }
    };
    ...
    useEffect(() => {
        ...
        const initData = async () => {
            try {
                const userRes = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/users/me`, { headers: { 'Authorization': `Bearer ${token}` } });
                ...
                const statsRes = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/doctor/stats`, { 
                    headers: { 'Authorization': `Bearer ${token}` } 
                });
                if (statsRes.ok) {
