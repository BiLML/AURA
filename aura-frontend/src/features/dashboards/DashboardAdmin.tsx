...
const DashboardAdmin: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
             ...
             const meRes = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/users/me`, { headers: { 'Authorization': `Bearer ${token}` } });
            ...
            await Promise.allSettled([
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/users`, { headers }).then(r => r.json()).then(d => setUserList((d.users || d || []).filter((u:User) => u.role !== 'admin'))),
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/clinics/admin/pending`, { headers }).then(r => r.json()).then(d => setClinicRequests(d.requests || [])),
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/clinics/`, { headers }).then(r => r.json()).then(d => {
                    setActiveClinics(d.filter((c:any) => c.status === 'APPROVED'));
                    setSuspendedClinics(d.filter((c:any) => c.status === 'SUSPENDED'));
                }),
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/reports`, { headers }).then(r => r.json()).then(d => setFeedbackList(d.reports || [])),
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/config`, { headers }).then(r => r.json()).then(d => setAiConfig(d)),
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/billing/packages`, { headers }).then(r => r.json()).then(d => setPackageList(d)),
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/stats/analytics`, { headers }).then(r => r.json()).then(d => setAnalyticsData(d)),
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/audit-logs`, { headers }).then(r => r.json()).then(d => setAuditLogs(d)),
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/templates`, { headers }).then(r => r.json()).then(d => setTemplates(d)),
                fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/stats/global`, { headers }).then(r => r.json()).then(data => {
                    setGlobalStats({
    ...
    const handleSaveUser = async () => {
        ...
        try {
            if (editForm.role !== editingUser.role) {
                await fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/users/${editingUser.id}/role`, {
                    method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                    body: JSON.stringify({ role: editForm.role })
                });
            }
            if (editForm.status !== editingUser.status) {
                await fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/users/${editingUser.id}/status`, {
                    method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    ...
    const handleToggleLock = async (user: User) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/users/${user.id}/status`, {
                method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    ...
    const handleClinicAction = async (clinicId: string, action: 'APPROVED' | 'REJECTED' | 'SUSPENDED' | 'ACTIVE') => {
        ...
        try {
            ...
            await fetch(`${import.meta.env.VITE_API_URL}/api/v1/clinics/admin/${clinicId}/status`, {
                method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    ...
    const handleCreatePackage = async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/billing/packages`, {
    ...
    const handleSaveTemplate = async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/admin/templates/${selectedTemplate.code}`, {
                method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
